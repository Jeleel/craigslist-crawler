...
try {
	...
	*/
	...
	if (!$html) {
		throw new Exception("$search_query seems unreachable.");
	}
	...
	if (count($postings) > 0) {
		echo "\n Let's check if these postings are within the bounds of the perimeter.";
	...
	$headers .= 'From: craigslist-script <craigscript@gmail.com>' . "\r\n";
	$headers .= 'To: ';
	foreach ($emails_arr as $email) {
		$headers .= "$email <$email>,";
	}
	...
	foreach ($valid_postings as $posting) {
		...
		// Mail it to comma separated emails
	...
	err("Exception occurred, sending notification email. error->". $e->getMessage());
	mail($emails, 'CRAIGSLIST SCRIPT - '.$email_subject_prefix, "<html><body><h3>".$e->getMessage()."</h3><pre>".$e->getTraceAsString()."</pre></body></html>", $headers);
}